...
8. [Accounts, stats & persistence](#8-accounts-stats--persistence)
...
The backend is a single Flask application (`backend/server.py`, ~1,569 lines).
Scanner state is in-memory (model artifacts, caches, host lists); everything
else that must persist — user accounts, scan history, threat reports, minigame
scores and per-user stats — lives in a **MongoDB** database (`phisdetect`),
reached via PyMongo using the `MONGO_URI` in `backend/.env`. The scanner is
...
| **Rule heuristics** | Brand-spoofing, cheap TLDs, suspicious keywords, IDN tricks, phishing phrases, From/Reply-To spoofing | Fast |
...
A third group of endpoints handles **accounts and persistence** (all
MongoDB-backed): session auth (`/api/auth/signup|login|logout|me|reward`),
threat reporting (`/api/report/threat`), the per-user dashboard
(`/api/user/stats`), and **minigame results** (phish-or-legit, link-dismantler,
threat-hunt) which are validated, saved to the `minigame_scores` collection,
trimmed to the top 20 per game/difficulty, and served through
...
The app connects to MongoDB at import time. `backend/.env` (git-ignored)
provides `MONGO_URI`, loaded by `python-dotenv`. The client is created with
`serverSelectionTimeoutMS=15000`; at startup the app pings the database and
creates unique indexes on `users.email` and `user_stats.email`. If the database
is unreachable, a warning is printed but the app **still starts** — the
scanners keep working and only the account / stats / leaderboard features are
unavailable (their endpoints return 503).

Database name: `phisdetect`. Five collections:

| Collection | Global | Purpose |
|------------|--------|---------|
| `users` | `_users` | accounts — `email` (unique), `name`, `password_hash`, `token` + `token_created`, `created_at` (also `points`/`reports`, see §8.1) |
| `user_stats` | `_stats` | per-user dashboard counters — `scans`, `threats`, `reports`, `minigamePoints` (`email` unique) |
| `minigame_scores` | `_score_entries` | one document per submitted minigame round; trimmed to top 20 per game/difficulty |
| `scans` | `_scan_events` | one document per recorded scan event (logged-in users) |
| `reports` | `_report_events` | one document per reported threat (logged-in users) |

Every DB call is wrapped in try/except; failures print to the log and degrade
to a neutral response (usually `503 {"error": "Database unavailable, please try
again"}`).
...
### 5.4 `domain_age(host, timeout=30) -> human string`

Runs `python-whois` inside a single-thread pool with a 30 s timeout (WHOIS
...
Each future gets up to 35 s to finish; stragglers fall back to their default.
(The endpoint caps the whole `enrich` call at 20 s — see §4.4.)
...
> **Dead code:** `grammar_level(content)` (line 772) maps `grammar_hits` to a
...
## 8. Accounts, stats & persistence (MongoDB)

Accounts, scan history, threat reports, minigame scores and per-user stats are
persisted in MongoDB (collections listed in §2.2).

### 8.1 Sessions & authentication

- Passwords are stored as **Werkzeug hashes** (`generate_password_hash`) and
  verified with `check_password_hash`; raw passwords are never stored or
  returned.
- A successful signup/login issues a **session token** (`secrets.token_hex(32)`,
  64 hex chars) stored on `users.token`. The client sends it as
  `Authorization: Bearer <token>`; `_current_user()` resolves it to a user
  document, and every authenticated endpoint returns `401 {"error": "Not logged
  in"}` when it is missing or unknown.
- Login **rotates** the token (`users.token` is overwritten); logout clears it
  (`users.token = None`) and always returns `{"ok": true}`.
- Signup validation: email must match `[^@\s]+@[^@\s]+\.[^@\s]+`, password must
  be ≥ 8 chars; an existing email returns `409`.
- The public user payload (`_public_user`) is `{email, name, points, reports,
  rank}`. **points = reports × 10 + minigamePoints**, computed from `user_stats`
  (`_user_points`), *not* from the `users` doc's fields. `rank` = 1 + the number
  of other users with a strictly higher computed points total (`_user_rank`).

> **Reward caveat:** `POST /api/auth/reward` increments the *users* doc's
> `points`/`reports` fields, but the public payload computes points/reports from
> `user_stats`, so rewarded values are **not** reflected in the displayed
> profile numbers.

### 8.2 Scan history & threat reports

- `_record_scan(type, risk)` runs at the end of both scan endpoints. It does
  nothing for anonymous scans; for a logged-in user it appends a document to
  `scans` (`{email, type, risk, verdict, ts}`) and bumps `user_stats`
  (`scans += 1`, plus `threats += 1` when the verdict isn't `safe`).
- `POST /api/report/threat` (logged-in) appends to `reports` (`{email, ts}`) and
  bumps `user_stats.reports` — worth +10 points under the points formula.
- `_ensure_stats(email)` lazily **seeds** a `user_stats` document from existing
  history when none exists (sum of the user's minigame scores → `minigamePoints`,
  report count → `reports`), so pre-existing scores still count towards points.
- `GET /api/user/stats` (logged-in) returns the counters and the latest 10
  activity items, newest first: reports show as `"Threat reported (+10 points)"`,
  scans as `"URL scan — Threat"` / `"EMAIL scan — Safe"` style messages.

### 8.3 Minigame persistence & leaderboard

`POST /api/minigame/result`:

- Validation: `game` must be in `MINIGAME_TYPES` (`phish-or-legit`,
  `link-dismantler`, `threat-hunt`), `difficulty` in `{"easy","normal","hard"}`
  (default `normal`); `score`/`total`/`bestStreak` coerced to ints with sane
  floors (`score ≥ 0`, `total ≥ 1`, `streak ≥ 0`); `name` truncated to 24 chars,
  default `"Guest"`.
- A process-local `threading.Lock` (`_minigame_lock`) serialises the
  read-compute-write so concurrent submissions can't corrupt the top-20 trim.
- `rank` = 1 + the number of existing entries in the same game/difficulty with a
  strictly higher `(score, streak)` — a single `count_documents` query — so it
  is the 1-indexed position against the *prior* board.
- Each entry is `{game, difficulty, name, email (if logged in), score, total,
  streak, ts}` with a UTC ISO timestamp. After inserting, the collection is
  trimmed to the **top 20** by `(score desc, streak desc)` — everything outside
  that window is deleted; `top` is the resulting entry count.
- If the submitter is logged in, the round's score is added to their lifetime
  `user_stats.minigamePoints` (`_bump_stats(email, minigamePoints=score)`); the
  `minigame_scores` entry itself keeps its `email` field for provenance.
- Response: `{"ok": true, "game": ..., "difficulty": ..., "rank": n, "top": n}`.
  A database failure returns `503 {"error": "Database unavailable, score not
  saved"}`.

`GET /api/minigame/leaderboard` returns the top **5** of each game (sorted) ×
difficulty bucket as `{name, score, total, streak, ts}` — the `email` field is
never exposed: `{"leaderboard": {game: {easy: [...], normal: [...], hard: [...]}}}`.

> **Legacy leftover:** `backend/data/minigame_scores.json` still exists on disk
> but is **no longer read or written** — minigame persistence moved to MongoDB.
...
### 9.3 Accounts — `/api/auth/*`

`signup` and `login` return a session token; the other three require
`Authorization: Bearer <token>` (else `401 {"error": "Not logged in"}`).

`POST /api/auth/signup`
```json
{ "email": "aditya@example.com", "password": "secret123", "name": "Aditya" }
```
Errors: `400 {"error":"Enter a valid email address"}` (email fails
`[^@\s]+@[^@\s]+\.[^@\s]+`) · `400 {"error":"Password must be at least 8 characters"}` · `409 {"error":"An account with this email already exists"}` · `503` DB unavailable.
Response: `{"token": "…64 hex chars…", "user": {"email": "...", "name": "Aditya", "points": 0, "reports": 0, "rank": 1}}`

`POST /api/auth/login`
```json
{ "email": "aditya@example.com", "password": "secret123" }
```
Errors: `401 {"error":"Invalid email or password"}` · `503`.
Response: `{"token": "…", "user": {...}}` (token is rotated).

`POST /api/auth/logout` — clears the session token. Response: `{"ok": true}`.

`GET /api/auth/me` — Response: `{"user": {...}}`.

`POST /api/auth/reward` — `{ "points": 25, "reports": 2 }` clamped to `[0,1000]`
/ `[0,100]`; `400` on invalid fields. Response: `{"user": {...}}`.
> Caveat: this increments the `users` doc, which the displayed profile doesn't
> read (see §8.1).

### 9.4 `POST /api/report/threat`

Logged-in only. Errors: `401 {"error":"Not logged in"}` · `503` DB unavailable.
Response: `{"ok": true}`. Appends a report event and bumps the user's stats (a
report is worth +10 points under the points formula).

### 9.5 `GET /api/user/stats`
...
```json
{
  ...
  "pointsEarned": 320,
  "activity": [
    { "type": "scan", "message": "URL scan — Safe", "ts": "...", "date": "..." },
    { "type": "threat", "message": "EMAIL scan — Threat", "ts": "...", "date": "..." },
    { "type": "report", "message": "Threat reported (+10 points)", "ts": "...", "date": "..." }
  ]
}
```
`pointsEarned = reportsSubmitted × 10 + minigamePoints`; `activity` is the last
10 events, newest first (`type` is `report` / `threat` / `scan`).

### 9.6 `POST /api/minigame/result`

```json
{ "game": "phish-or-legit", "difficulty": "hard", "score": 9, "total": 10, "bestStreak": 9, "name": "Player1" }
```

Errors: `400 {"error":"Invalid game"}` · `400 {"error":"Invalid difficulty"}` · `400 {"error":"Invalid score fields"}` · `503 {"error":"Database unavailable, score not saved"}`

Response: `{"ok": true, "game": "phish-or-legit", "difficulty": "hard", "rank": 2, "top": 20}`
(`rank` is against the prior board; `top` = entries kept after the top-20 trim;
if logged in, the score is also credited to `user_stats.minigamePoints`).

### 9.7 `GET /api/minigame/leaderboard`

Response: `{"leaderboard": {"link-dismantler": {"easy": [ ...up to 5 entries... ], "normal": [...], "hard": [...]}, ...}}`
Each entry is `{name, score, total, streak, ts}` (no emails exposed).
...
- **Localhost only.** The server binds `127.0.0.1:3000` — reachable only from
  the machine it runs on. The two scan endpoints and the leaderboard are open;
  the account / report / stats endpoints require `Authorization: Bearer <token>`
  (see §8.1). Scan endpoints themselves are unauthenticated by design.
...
- **Two hard fails.** If `phishing_model.joblib` or `features.txt` is missing,
  ...
  instead. MongoDB is a **soft** dependency — if it's down at startup the app
  still serves scans and only the account/stats/leaderboard endpoints fail
  (503).
...
- **MongoDB-backed state.** All persistent state lives in MongoDB
  (`phisdetect`), so the old single-writer JSON-file concern is gone. The
  process-local `_minigame_lock` only serialises the local read-modify-write;
  unique indexes on `users.email` and `user_stats.email` guard against duplicate
  accounts, and multi-process correctness relies on the database. The
  `MONGO_URI` credential lives only in `backend/.env` (git-ignored).
