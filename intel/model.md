...
10. [Dataset provenance & lineage](#10-dataset-provenance--lineage)
...
|---|------|------|-----------|-------|-----------|
| 1 | `model/trained/phishing_model.joblib` | ~403 MB | `model/src/train.py` | 103 hand-crafted URL features | Random Forest + isotonic calibration |
| 2 | `model/trained/url_text_model.joblib` | ~6.4 MB | `model/src/train_url_text_model.py` | The raw URL string as text | TF-IDF + Logistic Regression (custom char analyzer) |
| 3 | `model/trained/email_text_model.joblib` | ~4.7 MB | `model/src/train_email_text_model.py` | The raw email text (subject + body) | TF-IDF + Logistic Regression |
...
> one (403 MB) exceeds GitHub's 100 MB file limit. A fresh clone must retrain
...
- Samples 600 random domains from `model/lists/tranco_top1m.txt` (first 1,500,
  ...
  `ebay.com`, `walmart.com`, etc.). In the current run only 12 of the 18 needed
  appending (6 were already in the 600-domain sample), giving **612 distinct
  domains**.
...
(612 domains × 5 URLs = exactly 3,060 benign rows, produced with no feature-
extraction failures.)
...
1. `clean_url()`: strip and lowercase the input, then remove the scheme
   (`https://` → rest of string).
2. Cut the string at the first whitespace / comma / semicolon / quote / angle
   bracket (`[\s,;'"<>]`) so trailing markdown or HTML punctuation never
   reaches the model.
...
- **Modern synthetic additions** — always appended: 4,000 "modern phish"
  emails (account-suspended, payment-failed, unusual-activity, prize, delivery,
  DocuSign, HR, crypto, invoice templates from `_modern_phish`) and 4,000
  "modern benign" emails (sign-in reviews, verification codes, password resets,
  order confirmations, receipts, conference notices, team meeting notes,
  newsletters, blocked sign-in alerts from `_modern_benign`). Both generators
  use `random.Random(42)` so the synthetic rows are reproducible.
...
**Composition of the shipped 12,374-row file** (from the 2026-08-06 build; the
numbers come from re-running `update_email_data.py` with the same inputs):

- **Phish pool:** 3,100 unique real emails (`phishing0.mbox` 281,
  `phishing1.mbox` 325, `phishing2.mbox` 874, `phishing3.mbox` 1,620; dedup on
  the first 400 chars) **+ 4,000 modern synthetic = 7,100**, sampled down to
  `n = 6,187`.
- **Benign pool:** 2,187 real emails from `easy_ham.tar.bz2` (body > 200
  chars; capped at 2,500) **+ 4,000 modern synthetic = 6,187**, all of which
  was used (`n = 6,187`). The Enron fallback was **not** used — `enron.tgz` is
  not present on disk.
- Given the same corpora the build is **byte-for-byte deterministic** (modern
  generators use `random.Random(42)`, balancing uses `random.seed(42)`); two
  consecutive runs on 2026-08-11 produced identical files
  (md5 `98fc5d8231e1aef39bae9516e4d910ab`).
...
3. **Structural heuristic** — `heuristic_risk(host)` checks brand
   ...
   words in subdomains, cheap-TLD tricks, IDN/punycode labels, and suspicious
   keywords (`login`, `verify`, `secure`…). If any fire, it returns a heuristic probability
...
Because `train.py` fits and holds a ~403 MB model object in memory (plus the
...
|------|-------|------|------------|-----------|
| `model/trained/phishing_model.joblib` | Main (103 features) | ~403 MB | `train.py` | `server.py` |
| `model/trained/url_text_model.joblib` | URL text (TF-IDF, tuple) | ~6.4 MB | `train_url_text_model.py` | `server.py` |
| `model/trained/email_text_model.joblib` | Email text (TF-IDF pipeline) | ~4.7 MB | `train_email_text_model.py` | `server.py` |
...
and semantic evidence for both URLs and emails. For the full provenance of
every data artifact (sources, build dates, reproducibility), see
[Section 10](#10-dataset-provenance--lineage).

---

## 10. Dataset provenance & lineage

This section records, for every data artifact in `model/`, where it came from,
which script produced it, when, and whether it can be reproduced. It is the
authoritative counterpart to the training-data tables in Sections 2–4.

### 10.1 Lineage overview

```
external corpora                        model/ scripts                on-disk artifacts
────────────────────────────────────    ────────────────────────      ─────────────────────────────
tranco-list.eu/top-1m.csv.zip ───────►  update_lists.py            ─► lists/tranco_top1m.txt (+ top100k)
openphish.com/feed.txt        ───────►  update_lists.py            ─► lists/openphish_hosts.txt
                                                                        (→ backend ALLOW / BLOCK)
monkey.org phishing mboxes ──┐
spamassassin easy_ham ───────┼────────► update_email_data.py       ─► data/email_text_dataset.jsonl
modern synth (4,000+4,000) ──┘                                          (→ Model 3)
dataset_small.csv ────┐
tranco_top1m.txt ─────┴──────────────► augment_dataset.py          ─► trained/dataset_augmented.csv
                      (612 domains × 5 benign URLs)                      (→ Model 1)
OpenPhish + Phishing.Database (live) ► train_url_text_model.py          (→ Model 2; data rebuilt at train time)
```

### 10.2 Artifact-by-artifact provenance

| Artifact | Source / builder | Built (file date) | Size | Regenerable? |
|----------|------------------|-------------------|------|--------------|
| `trained/dataset_small.csv` | External URL dataset (no generating script; origin not recorded in the repo) | 2026-08-03 | 58,645 rows × 112 cols, ~15.3 MB | **No** |
| `trained/dataset_augmented.csv` | `augment_dataset.py` from `dataset_small.csv` + `tranco_top1m.txt` | 2026-08-11 (rebuilt; originally 2026-08-05) | 61,705 rows × 104 cols, ~14.2 MB | Yes, byte-for-byte from identical inputs |
| `lists/tranco_top1m.txt` | `update_lists.py` from `https://tranco-list.eu/top-1m.csv.zip` (host column, lowercased) | 2026-08-04 | 1M hosts, ~14.1 MB | Frozen snapshot; re-downloading differs (rankings update daily) |
| `lists/tranco_top100k.txt` | same download, first 100k | 2026-08-04 | 100k hosts, ~1.3 MB | Same as above |
| `lists/openphish_hosts.txt` | `update_lists.py` from the OpenPhish feed (`netloc` of each line) | 2026-08-04 | 267 hosts, 7 KB | Frozen snapshot; re-downloading differs |
| `data/email_text_dataset.jsonl` | `update_email_data.py` (Section 4.4) | 2026-08-11 (rebuilt; originally 2026-08-06) | 12,374 rows (~6,187 each), ~12.2 MB | Yes, byte-for-byte from the same corpora (verified: md5 `98fc5d8231e1aef39bae9516e4d910ab` across two runs) |
| `data/phishing0.mbox` … `phishing3.mbox` | Jose Nazario phishing corpus, `https://monkey.org/~jose/phishing/phishing{i}.mbox` | 2026-08-05 | 3.0 / 4.0 / 9.9 / 19.1 MB | Static source file |
| `data/easy_ham.tar.bz2` | SpamAssassin public corpus, `https://spamassassin.apache.org/old/publiccorpus/20021010_easy_ham.tar.bz2` | 2026-08-05 | 1.6 MB | Static source file |
| `lookup/tlds.txt` | Static lookup table (1,390 TLDs); source not recorded in the repo | 2026-08-03 | 10 KB | **No** |
| `lookup/shorteners.txt` | Static lookup table (443 shortener hosts); source not recorded in the repo | 2026-08-03 | 3 KB | **No** |

All raw source files above (`data/*`, `lists/*`, `lookup/*`, the two CSVs,
`features.txt`) are committed to git; only the three `.joblib` models are
ignored (Section 1).

### 10.3 External sources, terms & caveats

- **Tranco** (`tranco-list.eu`) — research-oriented top-sites ranking. Each
  list is the Dowdall-combined average of 30 days of rankings from Cisco
  Umbrella, Majestic (CC BY 3.0), Farsight, Google Chrome UX Report (CC BY-SA
  4.0) and Cloudflare Radar (CC BY-NC 4.0). Because the ranking updates daily,
  a re-download is a *different* snapshot than the frozen 2026-08-04 file.
- **OpenPhish feed** (`openphish.com/feed.txt`) — the scripts and Section 5.2
  still point at this URL, but OpenPhish has moved its free community feed to
  GitHub (`https://raw.githubusercontent.com/openphish/public_feed/refs/heads/main/feed.txt`,
  updated every 12 hours, limited URL count). Terms: **personal / non-commercial
  use only**; no redistribution; academic use requires attributing OpenPhish
  and recording the access date. **Check that the feed URL still resolves
  before retraining Model 2 or refreshing the lists.**
- **Phishing.Database** (`github.com/mitchellkrogza/Phishing.Database`,
  `phishing-domains-ACTIVE.txt`) — **MIT-licensed**; continuously re-validated
  with PyFunceble, so only currently-active phishing domains are kept.
- **SpamAssassin public corpus** (`spamassassin.apache.org/old/publiccorpus/`) —
  the 2002-10-10 `easy_ham` set (2,500 non-spam messages). Public-domain /
  freely redistributable; original message copyright remains with the senders;
  the corpus should **not** be injected into live mail systems (the pipeline
  only trains on it).
- **monkey.org phishing mboxes** (`monkey.org/~jose/phishing/`) — Jose
  Nazario's public phishing corpus, widely used in phishing research.

### 10.4 What is reproducible and what is not

| Artifact | Reproducibility |
|----------|-----------------|
| `dataset_augmented.csv` | Byte-for-byte from `dataset_small.csv` + the frozen `tranco_top1m.txt` (`random.seed(7)`); verified on 2026-08-11. |
| `email_text_dataset.jsonl` | Byte-for-byte from the frozen corpora + mboxes; verified on 2026-08-11 (identical md5 across two runs). |
| `lists/*` | Frozen snapshots; a re-download differs because the upstream sources update (Tranco daily, OpenPhish every 12 h). |
| Model 2 training set | **Not** reproducible — rebuilt from live feeds at training time; runs at different times differ. |
| `dataset_small.csv` | **Not** reproducible — no generating script or recorded source. |
| `lookup/tlds.txt`, `lookup/shorteners.txt` | **Not** reproducible — static tables with unrecorded origin. |

### 10.5 Handling & privacy notes

- Emails are reduced to a `subject: … \n body` text blob before training, and
  `clean_email()` erases concrete URLs, addresses and numbers into
  `URL`/`EMAIL`/`NUM` placeholders, so the trained model never sees raw
  personal data — but the raw corpora themselves are committed to the repo for
  regeneration.
- The SpamAssassin corpus readme asks that its messages never be re-sent into
  live email systems; the pipeline complies by using them only as training
  text.
