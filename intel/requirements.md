...
| Disk | ~450 MB free | The three trained `.joblib` files total ≈ 414 MB (`403 + 6.4 + 4.7` on disk) |
| RAM | ~1–2 GB | Loading the 403 MB Random Forest + `pandas`/`scikit-learn` at startup |
| Network | Outbound DNS + HTTPS + WHOIS + MongoDB | See [§6](#6-external-network-services) |
...
runs on and must not be exposed directly to the internet without a reverse
proxy. The two scan endpoints are unauthenticated by design; the account /
report / stats endpoints require a session bearer token (see `backendlogic.md`
§8.1).
...
`model/requirements.txt` — all **eleven** entries are **pinned with `==`** so
installs are reproducible:

| Package | Version | Why it is needed | Used by |
|---------|---------|------------------|---------|
| `Flask` | 3.1.3 | Web framework — serves the 11 JSON API endpoints | backend |
...
| `pymongo` | 4.17.0 | MongoDB driver — accounts, scan history, threat reports, minigame scores & user stats (the `phisdetect` DB) | backend |
| `python-dotenv` | 1.2.2 | Loads `MONGO_URI` from `backend/.env` at startup | backend |
...
The eleven entries pull in their own dependencies automatically. The important
...
| `python-dateutil`, `six`, `narwhals` | pandas 3.x | DataFrame internals (pandas 3.0 dropped `pytz`) |
...
|-----|-------------------|
| **Run the backend** (`python backend/server.py`) | all eleven |
...
only matters for minimal offline deployments. `pymongo` / `python-dotenv` are
backend-only (the trainers never touch the database).
...
Node.js, no bundler, and nothing to `npm install`**. It is served as-is. The
repo ships its own zero-dependency dev server, `frontend/serve.py` (uses only
`http.server`/`socketserver` from the stdlib), which binds `0.0.0.0:8000` and
sends no-cache headers so the browser always reloads the latest JS/CSS;
`python -m http.server` in `frontend/` also works.
...
|------|------|-----------|----------------------|
| `model/trained/phishing_model.joblib` | ~403 MB | **Yes** | **App crashes at startup** |
| `model/trained/features.txt` | ~1.7 KB | **Yes** | **App crashes at startup** |
| `model/trained/url_text_model.joblib` | ~6.4 MB | No | URL-text scoring silently disabled (`None`) |
| `model/trained/email_text_model.joblib` | ~4.7 MB | No | Email-text scoring silently disabled (`None`) |
...
### 5.1 `backend/.env` (MongoDB credential)

The backend reads `MONGO_URI` from `backend/.env` (git-ignored) via
`python-dotenv` and connects to the `phisdetect` database on MongoDB. If it is
missing or the database is unreachable, the app still starts and scanning keeps
working — only the account / stats / leaderboard features fail (503). Create it
as described in [§7](#7-setting-up-the-environment).
...
| MongoDB (e.g. `*.mongodb.net`) | MongoDB wire protocol over TLS (SRV/TCP 27017) | accounts, scan history, threat reports, minigame scores, user stats (the `phisdetect` DB) | signup/login, dashboard, leaderboard, logged-in scans & reports |
...
effectively heuristic-only. When MongoDB is unreachable the scanner still works;
only the DB-backed endpoints (accounts / stats / leaderboard) return 503.

---

## 7. Setting up the environment

From a fresh clone (the heavy `.joblib` files and `model/venv/` are git-ignored):

```bash
# 1. Create and populate the Python environment (the project venv lives at model/venv/)
python3.14 -m venv model/venv
model/venv/bin/python -m pip install -r model/requirements.txt

# 2. Provide MongoDB credentials (accounts / stats / leaderboard features)
#    Create backend/.env (git-ignored) and add your connection string:
#    MONGO_URI=mongodb+srv://<user>:<password>@<cluster>/...?retryWrites=true&w=majority

# 3. (Fresh clone only) generate the model artifacts
model/venv/bin/python model/src/train.py                  # phishing_model.joblib + features.txt
model/venv/bin/python model/src/train_url_text_model.py   # url_text_model.joblib
model/venv/bin/python model/src/train_email_text_model.py # email_text_model.joblib
# Optionally refresh the lists:
model/venv/bin/python model/src/update_lists.py

# 4. Start the API
model/venv/bin/python backend/server.py        # listens on 127.0.0.1:3000

# 5. Serve the frontend (stdlib dev server with no-cache; CORS is already enabled)
python3 frontend/serve.py                       # http://localhost:8000
```
