...
# ============================================
# MONGODB — credentials live in backend/.env (git-ignored)
# ============================================
load_dotenv(os.path.join(os.path.dirname(os.path.abspath(__file__)), ".env"))
_MONGO_URI = os.environ.get("MONGO_URI", "")
_mongo = pymongo.MongoClient(_MONGO_URI, serverSelectionTimeoutMS=15000)
_db = _mongo["phisdetect"]
_users = _db["users"]
_score_entries = _db["minigame_scores"]
_scan_events = _db["scans"]
_report_events = _db["reports"]
_stats = _db["user_stats"]

try:
    _db.command("ping")
    _users.create_index("email", unique=True)
    _stats.create_index("email", unique=True)
    print("[MongoDB] Connected to Atlas")
except Exception as exc:
    print(f"[MongoDB] WARNING: could not reach database ({exc}). "
...
def domain_age(host, timeout=30):
    def run():
        try:
...
def _age_cached(host):
    key = f"age:{host}"
    now = time.time()
    hit = CACHE.get(key)
    if hit and now - hit[0] < 1800:
        return hit[1]
    value = domain_age(host, timeout=30)
    if value != "Unknown":
        CACHE[key] = (now, value)
    return value
...
def enrich(url, host, https):
    ...
    try:
        futs = {
            "domainAge": ex.submit(lambda: _age_cached(host)),
...
# ============================================
# ACCOUNTS & AUTHENTICATION (email + password only)
# ============================================

def _public_user(u):
    email = u.get("email")
    points = _user_points(email)
    if points is None:
        points = u.get("points", 0)
        reports = u.get("reports", 0)
    else:
        try:
            reports = _ensure_stats(email).get("reports", 0)
        except Exception:
            reports = u.get("reports", 0)
    return {
        "email": email,
        "name": u.get("name", "Guest User"),
        "points": points,
        "reports": reports,
        "rank": _user_rank(points),
    }


def _user_rank(points):
    # Rank by the same computed points (reports*10 + minigame points) used in
    # the profile. The account's own 'points' field is never modified.
    try:
        better = _stats.count_documents({
            "$expr": {"$gt": [
                {"$add": [{"$multiply": [{"$ifNull": ["$reports", 0]}, 10]},
                          {"$ifNull": ["$minigamePoints", 0]}]},
                points,
            ]},
        })
        return better + 1
...
# ============================================
# USER STATS & DASHBOARD DATA (MongoDB persistence)
# ============================================

def _bump_stats(email, **fields):
    """Increment counter fields on the user's stats document (created on demand).

    The 'users' account collection is left untouched; all dashboard numbers
    live in the separate 'user_stats' collection."""
    if not email or not fields:
        return
    try:
        _stats.update_one(
            {"email": email},
            {"$inc": fields, "$setOnInsert": {"email": email}},
            upsert=True,
        )
    except Exception as exc:
        print(f"[MongoDB] failed to update stats: {exc}")


def _ensure_stats(email):
    """Return the user's stats document, seeding it once from existing history
    so pre-existing minigame scores and reports still count towards points."""
    if not email:
        return None
    try:
        stats = _stats.find_one({"email": email})
        if stats is not None:
            return stats
        mg = sum((e.get("score") or 0)
                 for e in _score_entries.find({"email": email}, {"score": 1}))
        reps = _report_events.count_documents({"email": email})
        try:
            _stats.update_one(
                {"email": email},
                {"$setOnInsert": {
                    "email": email, "scans": 0, "threats": 0,
                    "reports": reps, "minigamePoints": mg,
                }},
                upsert=True,
            )
        except pymongo.errors.DuplicateKeyError:
            pass
        return _stats.find_one({"email": email})
    except Exception as exc:
        print(f"[MongoDB] failed to ensure stats: {exc}")
        return None


def _user_points(email):
    """Points = 10 per reported threat + all minigame points earned."""
    stats = _ensure_stats(email)
    if stats is None:
        return None
    return (stats.get("reports", 0) * 10) + stats.get("minigamePoints", 0)


def _record_scan(scan_type, risk):
    """Persist a scan event for the logged-in user (safe scans count too)."""
    auth = request.headers.get("Authorization", "")
    print(f"[DEBUG] _record_scan({scan_type}, {risk}) auth_header={'present' if auth else 'MISSING'}", flush=True)
    u = _current_user()
    if not u:
        print("[DEBUG] _record_scan: no current user (NOT recording)", flush=True)
        return
    email = u["email"]
    try:
        _scan_events.insert_one({
            "email": email,
            "type": scan_type,
            "risk": risk,
            "verdict": VERDICT_LABELS.get(risk, risk),
            "ts": datetime.now(timezone.utc).isoformat(),
        })
        if risk != "safe":
            _bump_stats(email, scans=1, threats=1)
        else:
            _bump_stats(email, scans=1)
    except Exception as exc:
        print(f"[MongoDB] failed to record scan: {exc}")
...
def _current_user():
    ...
    if auth.startswith("Bearer "):
        try:
            return _users.find_one({"token": auth[7:]})
        except Exception:
            return None
    return None
...
def auth_signup():
    ...
    name = str(data.get("name") or "").strip()[:24] or email.split("@")[0]
    ...
    try:
        ...
        return jsonify({"token": token, "user": _public_user(_users.find_one({"email": email}))})
    except pymongo.errors.DuplicateKeyError:
        return jsonify({"error": "An account with this email already exists"}), 409
    except Exception as exc:
        print(f"[MongoDB] signup failed: {exc}")
...
def auth_login():
    ...
    try:
        ...
        _users.update_one(
            {"_id": u["_id"]},
            {"$set": {"token": token, "token_created": datetime.now(timezone.utc).isoformat()}},
        )
        u["token"] = token
        return jsonify({"token": token, "user": _public_user(u)})
    except Exception as exc:
        print(f"[MongoDB] login failed: {exc}")
        return jsonify({"error": "Database unavailable, please try again"}), 503
...
def auth_logout():
    ...
    if u:
        try:
            _users.update_one({"_id": u["_id"]}, {"$set": {"token": None}})
        except Exception:
            pass
...
@app.post("/api/auth/reward")
def auth_reward():
    """Award points and/or reports to the logged-in user's account."""
    u = _current_user()
    if not u:
        return jsonify({"error": "Not logged in"}), 401
    data = request.get_json(silent=True) or {}
    try:
        points = min(max(int(data.get("points", 0)), 0), 1000)
        reports = min(max(int(data.get("reports", 0)), 0), 100)
    except (TypeError, ValueError):
        return jsonify({"error": "Invalid reward fields"}), 400
    try:
        _users.update_one({"_id": u["_id"]}, {"$inc": {"points": points, "reports": reports}})
        return jsonify({"user": _public_user(_users.find_one({"_id": u["_id"]}))})
    except Exception as exc:
        print(f"[MongoDB] reward failed: {exc}")
        return jsonify({"error": "Database unavailable, please try again"}), 503


@app.post("/api/report/threat")
def report_threat():
    """Record a threat report for the logged-in user (+10 points implied)."""
    u = _current_user()
    if not u:
        return jsonify({"error": "Not logged in"}), 401
    email = u["email"]
    try:
        _report_events.insert_one({
            "email": email,
            "ts": datetime.now(timezone.utc).isoformat(),
        })
        _bump_stats(email, reports=1)
    except Exception as exc:
        print(f"[MongoDB] report failed: {exc}")
        return jsonify({"error": "Database unavailable, please try again"}), 503
    return jsonify({"ok": True})
...
def user_stats():
    ...
    try:
        ...
        if stats is None:
            return jsonify({"error": "Database unavailable, please try again"}), 503
        scans = list(_scan_events.find({"email": email}, {"_id": 0})
                     .sort("ts", -1).limit(10))
        reports = list(_report_events.find({"email": email}, {"_id": 0})
                       .sort("ts", -1).limit(10))
    except Exception as exc:
        print(f"[MongoDB] stats failed: {exc}")
        return jsonify({"error": "Database unavailable, please try again"}), 503
    ...
    for r in reports:
        activity.append({
            "type": "report",
            "message": "Threat reported (+10 points)",
            "ts": r.get("ts", ""),
            "date": r.get("ts", ""),
        })
    for s in scans:
        ...
        activity.append({
            "type": "threat" if risk != "safe" else "scan",
            "message": "{} scan — {}".format(
                str(s.get("type", "url")).upper(),
                s.get("verdict", risk.capitalize())),
            "ts": s.get("ts", ""),
            "date": s.get("ts", ""),
        })
    activity.sort(key=lambda a: a.get("date", ""), reverse=True)

    return jsonify({
        "totalScans": stats.get("scans", 0),
        "threatsDetected": stats.get("threats", 0),
...
# ============================================
# MINIGAME RESULTS (MongoDB persistence)
# ============================================
...
def minigame_result():
    ...
    with _minigame_lock:
        try:
            better = _score_entries.count_documents({
                "game": game, "difficulty": difficulty,
                "$or": [{"score": {"$gt": score}},
                        {"score": score, "streak": {"$gt": streak}}],
            })
            rank = better + 1
            _score_entries.insert_one({
                "game": game,
            ...
            top_ids = [e["_id"] for e in _score_entries.find(
                {"game": game, "difficulty": difficulty},
                {"_id": 1},
            ).sort([("score", -1), ("streak", -1)]).limit(MINIGAME_TOP_N)]
            _score_entries.delete_many(
                {"game": game, "difficulty": difficulty, "_id": {"$nin": top_ids}})
            ...
            print(f"[MongoDB] minigame result failed: {exc}")
