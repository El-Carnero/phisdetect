...
function mgSubmitResult(game, difficulty, score, total, bestStreak, name) {
    try {
        ...
        if (window.AuthManager && AuthManager.token) {
        ...
        fetch(`${MINIGAME_BACKEND}/api/minigame/result`, {
            ...
            // Profile points are computed from minigame + report points on the
            // server, so re-sync the dropdown after a finished round.
            if (res && res.ok && window.ProfileManager
                    && typeof window.ProfileManager.refreshFromServer === 'function') {
                window.ProfileManager.refreshFromServer();
            }
        }).catch(() => {});
...
const MinigamesManager = {
    ...
    bank: [
        ...
        {
            kind: 'url', difficulty: 'easy', answer: 'legit',
            content: 'https://github.com/El-Carnero',
    ...
    awardPoints() {
        ...
        if (window.ProfileManager) {
            window.ProfileManager.addPoints(pts);
        }
...
const LinkDismantlerManager = {
    ...
    bank: [
        ...
        {
            difficulty: 'easy',
            url: 'https://github.com/El-Carnero',
            correct: 'github.com',
            distractors: ['github.com/El-Carnero', 'El-Carnero.com', 'github.login'],
            explain: 'Real GitHub — "github.com/El-Carnero" is a path, not a domain.'
        },
        {
            difficulty: 'easy',
...
const ThreatHuntManager = {
    ...
    bank: [
        ...
        {
            kind: 'url', difficulty: 'easy',
            chunks: [{ t: 'https://', f: false }, { t: 'github.com', f: false }, { t: '/', f: false }, { t: 'El-Carnero', f: false }],
            flags: [],
