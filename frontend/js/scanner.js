...
const ScannerManager = {
    ...
    submitReport(type, result) {
        Utils.confirmDialog(
            ...
            const loggedIn = !!(window.AuthManager && AuthManager.isLoggedIn);
            const onReported = () => {
                // Add notification
                ...
                if (loggedIn) {
                    // Points are computed on the server (reports * 10 + minigames);
                    // re-sync the profile and dashboard so they reflect the +10.
                    if (window.ProfileManager) window.ProfileManager.refreshFromServer();
                    if (window.DashboardManager) window.DashboardManager.refresh();
                } else {
                    // Guests keep local tracking.
                    this.addPoints(10);
                }
            };

            if (loggedIn) {
                this.callBackend('/api/report/threat', {})
                    .then(onReported)
                    .catch(err => this.showAlert(`Report failed: ${err.message}`, 'error'));
            } else {
                onReported();
            }
        });
    ...
    // ============================================
    addPoints(points) {
        // Update profile if available (syncs to the account when logged in)
        if (window.ProfileManager) {
            window.ProfileManager.addPoints(points, points === 10 ? 1 : 0);
        }
